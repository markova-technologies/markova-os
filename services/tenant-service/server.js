...
app.get('/api/tenant/webhooks', async (req, res) => {
  try {
    const ctx = req.securityContext;
    const result = await tenantDb.query(ctx, 
      'SELECT id, environment, webhook_url, created_at, updated_at FROM developer_webhooks WHERE company_id = $1 ORDER BY created_at DESC',
      [ctx.tenantId]
    );
    res.json(result.rows);
  } catch (error) {
...
app.delete('/api/tenant/webhooks/:environment', async (req, res) => {
  try {
    const ctx = req.securityContext;
    const result = await tenantDb.query(ctx,
      `DELETE FROM developer_webhooks WHERE environment = $1 AND company_id = $2 RETURNING id`,
