...
async def lifespan(app: FastAPI):
    ...
    # Start TTS audio cache cleanup task (runs every 6 hours)
    cleanup_task = asyncio.create_task(cleanup_audio_cache())
...
async def handle_inbound_call(
    ...
    """
    # Validate Twilio signature to block spoofed requests
    form_params = {"To": To, "From": From, "CallSid": CallSid}
    if not verify_twilio_signature(request, form_params):
        print(f"🚫 Rejected fake inbound call — invalid Twilio signature from {request.client.host}")
...
async def handle_speech_response(
    ...
    form_params = {"CallSid": CallSid, "SpeechResult": SpeechResult, "Confidence": Confidence}
    if not verify_twilio_signature(request, form_params):
        print(f"🚫 Rejected fake speech response — invalid Twilio signature")
        raise HTTPException(status_code=403, detail="Invalid Twilio signature")
...
<Response>
    ...
    # RAG Injection — prepend into system prompt (not mid-history) to avoid confusing LLMs
